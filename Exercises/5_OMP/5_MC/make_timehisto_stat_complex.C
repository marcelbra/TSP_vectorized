...
const int maxNAll = 100;  // max number of threads set to run
...
TString FileNameTemplate = "speedUpPi";
...
//     iFile=1;
    ...
    int Nall = 32; // number of threads sets to run
    Nstat = 1;
    //iff >> Nall;
    cout << "Begin read " << Nall << " threads sets data. With statistic: " << endl;
    ...
    for (int i=0; (i < Nall) && (!iff.eof()); i++){
      for (int j=0; j<Nstat;  j++){
        ...
        iff>>timeData[i].realTime.stat[j];
       // cout << timeData[i].realTime.stat[j] << endl;
          timeData[i].cpuTime.stat[j] =timeData[i].realTime.stat[j];
        flag = 1;
      }
      if (flag && (nTread != i+1)){
        break;
...
//     hrAll[iFile] = new TH1F("Real Time","Real Time",Nall+1,-0.5,Nall+0.5);  // all 1 .. end
    hrAll[iFile] = new TH1F("Real Time","Real Time",Nall+1,-0.5,Nall+0.5);  // only 2,4,6, .. end
    TH1F *hr = hrAll[iFile];
...
//   }
    for (int i=0; i<Nall; i++){  // only 2,4,6, .. end
      hr->SetBinContent(i+2,timeData[0].realTime.stat[0]/timeData[i].realTime.stat[0]);
      hr->SetBinError(i+2, timeData[i].realTime.error);
    }
...
//     hc->SetLineWidth(2);
  ...
  TCanvas *c1 = new TCanvas("OpenMP", "OpenMP", 700, 700);
...
//     hr->SetMarkerSize(1.5 +.5*i);
    ...
    hr->GetYaxis()->SetTitle(Form("Speed-up factor"));
    hr->GetYaxis()->SetRangeUser(0,32); // tr/us
