...
//     checkErr(ret, "clCreateSubDevices");
    ...
    // Build the program
    ret = clBuildProgram(program, 1, &device_id, "-x clc++", NULL, NULL);
...
//    size_t local_item_size = NCopy/4; // Process in groups of 64
    size_t local_item_size = localItemSize; // Process in groups of 64
...
//   cout << "Begin NowIs(h:m:s) " << seconds/3600 << ":"  << (seconds/60)%60 << ":" << seconds%60 << endl;
    ...
    if ( string( argv[0] ).find( "opencl" ) != string::npos ) {
        if ( argc != 2 ) {
            cout << "Please specify local item size for GPU" << endl;
            return 1;
        }
        localItemSize = atoi( argv[1] );
        tasks = 128-128%localItemSize;
    }
