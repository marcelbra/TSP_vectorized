...
TString FileNameTemplate = "make_opencltime";
...
void make_timehisto_stat_complex_opencl()
{
  TH1F *hrAll[nFiles];
  FileNamesInit();
...
//     iFile=1;
    ...
    iff >> Nall >> Nstat;
    cout << "Begin read " << Nall << " threads sets data. With statistic: " << Nstat << endl;
...
//   }
    for (int i=0; i<Nall; i++){  // only 2,4,6, .. end
      hr->SetBinContent(i+2,timeData[i].realTime.mean);
