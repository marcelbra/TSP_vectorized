// TODO: CHECK CONSTRUCTORS!!! (Robert Rueger, 2012-06-30 03:54)
...
#define ZERO        0.0f
#define ONE         1.f
#define c_light     0.000299792458f
#define c_light_i   3335.64095f

struct FieldVector {
    float4 X, Y, Z;
};

void FieldVector_Combine( struct FieldVector* fv, const struct FieldVector* H, const float4* w )
{
    fv->X += *w * ( H->X - fv->X );
    fv->Y += *w * ( H->Y - fv->Y );
    fv->Z += *w * ( H->Z - fv->Z );
...
struct FieldSlice {
    float4 X[10], Y[10], Z[10]; // polinom coeff.
    //float4 z;
};

struct FieldSlice FieldSlice_Constructor() {
    struct FieldSlice fs;
    for ( int i = 0; i < 10; i++ ) {
        fs.X[i] = fs.Y[i] = fs.Z[i] = 0;
    }
    return fs;
}

void FieldSlice_GetField_1( const struct FieldSlice* fs, const float4* x, const float4* y, float4* Hx, float4* Hy, float4* Hz )
{
    ...
    *Hx = fs->X[0] + fs->X[1] * *x + fs->X[2] * *y + fs->X[3] * x2 + fs->X[4] * xy + fs->X[5] * y2 + fs->X[6] * x3 + fs->X[7] * x2y + fs->X[8] * xy2 + fs->X[9] * y3;
    //+ X[10]*x4 + X[11]*x3y +X[12]*x2y2 +X[13]*xy3 + X[14]*y4;
    *Hy = fs->Y[0] + fs->Y[1] * *x + fs->Y[2] * *y + fs->Y[3] * x2 + fs->Y[4] * xy + fs->Y[5] * y2 + fs->Y[6] * x3 + fs->Y[7] * x2y + fs->Y[8] * xy2 + fs->Y[9] * y3;
    //+ Y[10]*x4 + Y[11]*x3y +Y[12]*x2y2 +Y[13]*xy3 + Y[14]*y4;
    *Hz = fs->Z[0] + fs->Z[1] * *x + fs->Z[2] * *y + fs->Z[3] * x2 + fs->Z[4] * xy + fs->Z[5] * y2 + fs->Z[6] * x3 + fs->Z[7] * x2y + fs->Z[8] * xy2 + fs->Z[9] * y3;
    //+ Z[10]*x4 + Z[11]*x3y +Z[12]*x2y2 +Z[13]*xy3 + Z[14]*y4;
}

void FieldSlice_GetField_2( const struct FieldSlice* fs, const float4* x, const float4* y, struct FieldVector* H )
{
    FieldSlice_GetField_1( fs, x, y, &( H->X ), &( H->Y ), &( H->Z ) );
}


struct FieldRegion {
    float4 x0, x1, x2 ; // Hx(Z) = x0 + x1*(Z-z) + x2*(Z-z)^2
    float4 y0, y1, y2 ; // Hy(Z) = y0 + y1*(Z-z) + y2*(Z-z)^2
    float4 z0, z1, z2 ; // Hz(Z) = z0 + z1*(Z-z) + z2*(Z-z)^2
    float4 z;
};

struct FieldRegion FieldRegion_Constructor() {
    struct FieldRegion fr;
    fr.x0 = fr.x1 = fr.x2 = fr.y0 = fr.y1 = fr.y2 = fr.z0 = fr.z1 = fr.z2 = fr.z = 0.f;
    return fr;
}

void FieldRegion_Set( struct FieldRegion* fr,
                      const struct FieldVector* H0, const float4* H0z,
                      const struct FieldVector* H1, const float4* H1z,
                      const struct FieldVector* H2, const float4* H2z )
{
    fr->z = *H0z;
    float4 dz1 = *H1z - *H0z, dz2 = *H2z - *H0z;
    float4 det = 1.f / ( dz1 * dz2 * ( dz2 - dz1 ) );
    float4 w21 = -dz2 * det;
    float4 w22 = dz1 * det;
    float4 w11 = -dz2 * w21;
    float4 w12 = -dz1 * w22;

    float4 dH1 = H1->X - H0->X;
    float4 dH2 = H2->X - H0->X;
    fr->x0 = H0->X;
    fr->x1 = dH1 * w11 + dH2 * w12 ;
    fr->x2 = dH1 * w21 + dH2 * w22 ;

    dH1 = H1->Y - H0->Y;
    dH2 = H2->Y - H0->Y;
    fr->y0 = H0->Y;
    fr->y1 = dH1 * w11 + dH2 * w12 ;
    fr->y2 = dH1 * w21 + dH2 * w22  ;

    dH1 = H1->Z - H0->Z;
    dH2 = H2->Z - H0->Z;
    fr->z0 = H0->Z;
    fr->z1 = dH1 * w11 + dH2 * w12 ;
    fr->z2 = dH1 * w21 + dH2 * w22 ;
}

void FieldRegion_Shift( struct FieldRegion* fr, const float4* Z0 )
{
    float4 dz = *Z0 - fr->z;
    float4 x2dz = fr->x2 * dz;
    float4 y2dz = fr->y2 * dz;
    float4 z2dz = fr->z2 * dz;
    fr->z = *Z0;
    fr->x0 += ( fr->x1 + x2dz ) * dz;
    fr->x1 += x2dz + x2dz;
    fr->y0 += ( fr->y1 + y2dz ) * dz;
...
struct Station {
    float4 z, thick, zhit, RL,  RadThick, logRadThick,
           Sigma, Sigma2, Sy;
    struct FieldSlice Map;
};

struct Station Station_Constructor() {
    struct Station s;
    s.Sigma = 20.E-4f;
    s.Sigma2 = s.Sigma * s.Sigma;

    // added the constructor because it would be called in the C++ code ...
    s.Map = FieldSlice_Constructor();
...
struct TrackV {
    struct HitV vHits[12];
    float4 T[6]; // x, y, tx, ty, qp, z
    struct CovV C;    // cov matr.
    float4 Chi2;
    float4 NDF;
};
...
// unitimer2[0].Start();
    const float
    c1 = 1.f, c2 = 2.f, c3 = 3.f, c4 = 4.f, c6 = 6.f, c9 = 9.f, c15 = 15.f, c18 = 18.f, c45 = 45.f, c2i = 1.f / 2.f, c3i = 1.f / 3.f, c6i = 1.f / 6.f, c12i = 1.f / 12.f;
...
// cout << "2Fil: Cycles = " << unitimer2[2].Cycles() << " ";
    ...
    float4 Syyy;
    {
        const float
        d = 1.f / 181440.f,
        c000 =   7560.f * d, c001 = 9.f * 1008.f * d, c002 = 5.f * 1008.f * d,
        c011 = 21.f * 180.f * d, c012 = 24.f * 180.f * d, c022 =  7.f * 180.f * d,
        c111 =    540.f * d, c112 =    945.f * d, c122 =    560.f * d, c222 = 112.f * d;
        float4 Fy22 = Fy2 * Fy2;
...
void Filter( struct TrackV* track, struct HitInfo* info, float4* u, float4* w )
{

    // convert input
    float4* T = track->T;
    struct CovV* C = &(track->C);
...
void FilterFirst( struct TrackV* track, struct HitV* hit, struct Station* st )
{
...
void AddMaterial( struct TrackV* track, struct Station* st, float4* qp0 )
{
    const float mass2 = 0.1396f * 0.1396f;
    ...
    const float c1 = 0.0136f, c2 = c1 * 0.038f, c3 = c2 * 0.5f, c4 = -c3 / 2.0f, c5 = c3 / 3.0f, c6 = -c3 / 4.0f;
...
void GuessVec( struct TrackV* t, struct Station* vStations, int NStations )
{
    float4* T = t->T;
    ...
    Int_t i = NHits - 1;
    z0 = vStations[i].zhit;
    struct HitV* hlst = &( t->vHits[i] );
    w = hlst->w;
    A0 = w;
    a0 = w * hlst->x;
    b0 = w * hlst->y;
    struct HitV* h = t->vHits;
    struct Station* st = vStations;
    for ( ; h != hlst; h++, st++ ) {
        x = h->x;
        y = h->y;
        w = h->w;
...
//          printf("cl: x =%f \n y= %f \n z= %f \n w=%f \n", x , y, z0, st->zhit);
    ...
    float4 det = 1.f / ( -A2 * A3A3 + A1 * ( A3A4 + A3A4 - A1A5 ) + A0 * ( A2A5 - A4A4 ) );
...
void Fit( struct TrackV* t, struct Station vStations[], int NStations )
{
// Stopwatch timer_test1,timer_test11,timer_test2;
// timer_test1.Start();
    struct HitInfo Xinfo, Yinfo;
    const float c16 = 16.f;
    Xinfo.cos_phi = ONE;
    Xinfo.sin_phi = ZERO;
...
// timer_test11.Start();
    ...
    struct FieldRegion f = FieldRegion_Constructor();
    float4 z0, z1, z2, dz;
    struct FieldVector H0, H1, H2;

    float4 qp0 = t->T[4];
    Int_t i = NStations - 1;
    struct HitV* h = &(t->vHits[i]);

    FilterFirst( t, h, &vStations[i] );
    AddMaterial( t, &vStations[i], &qp0 );

    z1 = vStations[ i ].z;
    FieldSlice_GetField_2( &(vStations[i].Map), &(t->T[0]), &(t->T[1]), &H1 );
    FieldVector_Combine( &H1, &(h->H), &(h->w) );

    z2 = vStations[ i - 2 ].z;
    dz = z2 - z1;

    // added temporaries because we have to pass pointers into GetField_2
    {
        float4 temp1 = t->T[0] + t->T[2]*dz;
        float4 temp2 = t->T[1] + t->T[3]*dz;
        FieldSlice_GetField_2( &(vStations[ i - 2 ].Map), &temp1 , &temp2, &H2 );
    }

    h = &(t->vHits[i - 2]);
    FieldVector_Combine( &H2, &(h->H), &(h->w) );
...
// Stopwatch  unitimer2[3];
    for ( --i; i >= 0; i-- ) {
        h = &(t->vHits[i]);
        struct Station* st = &vStations[i];
        z0 = st->z;
        dz = ( z1 - z0 );

        // added temporaries for the same reason as above ...
        {
            float4 temp1 = t->T[0] - t->T[2]*dz;
            float4 temp2 = t->T[1] - t->T[3]*dz; 
            FieldSlice_GetField_2( &(st->Map), &temp1, &temp2, &H0 );
        }

        FieldVector_Combine( &H0, &(h->H), &(h->w) );
        FieldRegion_Set( &f, &H0, &z0, &H1, &z1, &H2, &z2 );
...
__kernel void vector_add( __global struct TrackV* t,
                          __global struct Station* vStations,
                          __global int* NStations,
                          __global int* NFits )
{
    int i = get_global_id( 0 );
    struct TrackV curTrack = t[i];
    struct Station vStations_local[10];
    int iStation = 0;
    for ( iStation = 0; iStation < *NStations; iStation++ ) {
        vStations_local[iStation] = vStations[iStation];
        // printf("cl: %f %f \n", vStations_local[iStation].z.x, vStations[iStation].z.x);
        // printf("cl: %f %f \n", vStations_local[iStation].thick.x, vStations[iStation].thick.x);
        // printf("cl: %f %f \n", vStations_local[iStation].zhit.x, vStations[iStation].zhit.x);
        // printf("cl: %f %f \n", vStations_local[iStation].RL.x, vStations[iStation].RL.x);
        // printf("cl: %f %f \n", vStations_local[iStation].RadThick.x, vStations[iStation].RadThick.x);
        // printf("cl: %f %f \n", vStations_local[iStation].logRadThick.x, vStations[iStation].logRadThick.x);
        // printf("cl: %f %f \n", vStations_local[iStation].Sigma2.x, vStations[iStation].Sigma2.x);
        // printf("cl: %f %f \n", vStations_local[iStation].Sigma.x, vStations[iStation].Sigma.x);
        // printf("cl: %f %f \n", vStations_local[iStation].Sy.x, vStations[iStation].Sy.x);
    }
    int iTimes = 0;

    for ( iTimes = 0; iTimes < *NFits; iTimes++ ) {
        Fit( &curTrack, vStations_local, *NStations );
    }
//  if(i==0) printf("cl: %f %f %f %f ", curTrack.T[0].x, curTrack.T[5].x, curTrack.C.C00.x, curTrack.C.C44.x);
