...
struct FieldVector{
  float4 X, Y, Z;
  void Combine( FieldVector &H, const float4 &w ){
    X+= w*(H.X-X);
    Y+= w*(H.Y-Y);
    Z+= w*(H.Z-Z);
  }
...
struct FieldSlice{
  ...
  void GetField( const float4 &x, const float4 &y, float4 &Hx, float4 &Hy, float4 &Hz ){
  ...
  void GetField( const float4 &x, const float4 &y, FieldVector &H ){
...
struct FieldRegion{
  ...
  void Set( const FieldVector &H0, const float4 &H0z, 
	    const FieldVector &H1, const float4 &H1z, 
	    const FieldVector &H2, const float4 &H2z){
    z = H0z;
    float4 dz1 = H1z-H0z, dz2 = H2z-H0z;
    float4 det = 1./(dz1*dz2*(dz2-dz1));
    float4 w21 = -dz2*det;
    float4 w22 = dz1*det;
    float4 w11 = -dz2*w21;
    float4 w12 = -dz1*w22;
...
__kernel void vector_add( __global TrackV *t,
                          __global Station *vStations,
                          __global int *NStations,
                          __global int *NFits)
{
  int i = get_global_id(0);
  TrackV curTrack = t[i];
  Station vStations_local[10];
  int iStation = 0;
