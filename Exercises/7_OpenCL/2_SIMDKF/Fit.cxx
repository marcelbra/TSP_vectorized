...
//     }
    ...
    //TODO write your code here: get a platform, a CPU device, create subdevices (use CL_DEVICE_PARTITION_EQUALLY), 
    //create a context, a command queue, memory buffers on the device, copy data to the buffers, 
    //create a program from the kernel source, build the program, check the logs, create a kernel, set its arguments,
    //execute the kernel, read the fitted tracks, clean up the memory
...
//   cout << "Begin NowIs(h:m:s) " << seconds/3600 << ":"  << (seconds/60)%60 << ":" << seconds%60 << endl;
  ...
  if(string(argv[0]).find("omp") != string::npos || string(argv[0]).find("tbb") != string::npos ||
            string(argv[0]).find("opencl") != string::npos){
      if(argc != 2){
