...
#define COPY
#ifdef KNF
const int NCopy = 8000;
#endif
#ifdef DOUBLE_PRECISION_KNF
const int NCopy = 800;
#endif
#ifdef X5680
const int NCopy = 10000;
#endif
#endif
...
//       }
    ...
    for(int it=vecN; it<Fvec_t::Size; it++ ){
      for( int ista=0; ista<NStations; ista++ ){
        hxmem[ista][it] = hxmem[ista][0];
        hymem[ista][it] = hymem[ista][0];
        hwmem[ista][it] = 1.;
      }
    }
    for( int ista=0; ista<NStations; ista++ ){
...
//  vStations = Vc::malloc<Station, Vc::AlignOnVector>(8);
  ...
  if((string(argv[0]).find("singleVc") != string::npos) || (string(argv[0]).find("avxVc") != string::npos))
  {
    if(argc == 2)
    {
      if(atoi( argv[1] ) > vecN)
      {
        std::cout << "Too large vector size"<<std::endl;
        return 1;
      }
