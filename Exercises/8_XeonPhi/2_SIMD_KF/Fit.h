...
inline __attribute__((always_inline)) void Filter( TrackV &track, HitInfo &info, Fvec_t &u, Fvec_t &w )
{
...
#ifdef X87
  zetawi = w* zeta *rcp( (float)((int)initialised & (int)info.sigma2) + HCH);
  track.Chi2 += (float)((int)initialised & (int)(zeta * zetawi));    
#elif defined VC
...
inline __attribute__((always_inline)) void FilterFirst( TrackV &track, HitV &hit, Station &st )
{
...
inline __attribute__((always_inline)) void AddMaterial( TrackV &track, Station &st, Fvec_t &qp0 )
{
...
inline __attribute__((always_inline)) void GuessVec( TrackV &t, Station *vStations, int NStations )
{
  Fvec_t *T = t.T;
  ...
  T[1] = (  A2*b0 - A1*b1 )*det;
  T[3] = ( -A1*b0 + A0*b1 )*det;
  T[4] = -L*c_light_i*1.f/(txtx1 +T[3]*T[3]); //TODO 1.f/ is faster then rsqrt on MIC
  T[5] = z0;
}

inline __attribute__((always_inline)) void Fit( TrackV &t, Station vStations[], int NStations )
{
