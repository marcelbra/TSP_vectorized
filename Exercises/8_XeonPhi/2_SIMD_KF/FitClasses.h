...
# define ALIGNMENT_NONE

#elif defined( VALARRAY )

#include <valarray>
#include "openlab_mod/ValVec.h"
 int vecN = ValVec::VEC_SIZE;
typedef ValVec Fvec_t;
typedef float Single_t;
# define ALIGNMENT_NONE

#elif defined( LRB )

#include "openlab_mod/F32LrbVec.h"
typedef F32LrbVec Fvec_t;
 int vecN = F32LrbVec::VECSIZE;
typedef float Single_t;
# define ALIGNMENT_64
...
#include "Vctypedef.h"
typedef float Single_t;
#include "openlab_mod/align64.h"
#else
#include "openlab_mod/KNF_F32vec16.h"
typedef F32vec16 Fvec_t;
 int vecN = 16;
typedef float Single_t;
# define ALIGNMENT_64
#endif

#elif defined( DOUBLE_PRECISION_KNF )

#include "openlab_mod/KNF_F64vec8.h"
typedef F64vec8 Fvec_t;
 int vecN = 8;
typedef double Single_t;
# define ALIGNMENT_64

#elif defined( SIMPLSIMD )
...
#elif defined( NOTSIMD )

#include "openlab_mod/PSEUDO_F32vec4.h"
typedef F32vec4 Fvec_t;
 int vecN = 4;
typedef float  Single_t;
...
struct FieldVector{
  Fvec_t X, Y, Z;
  void Combine( FieldVector &H, const Fvec_t &w ){
    X+= w*(H.X-X);
    Y+= w*(H.Y-Y);
    Z+= w*(H.Z-Z);
  }
} __attribute__ ((aligned(sizeof(Fvec_t))));
...
struct Station
#ifndef ALIGNMENT_NONE
 : public ALIGNMENT_ALLOCATOR 
#endif
{
  Fvec_t z, thick, zhit, RL,  RadThick, logRadThick;
  Fvec_t Sigma, Sigma2, Sy;
  FieldSlice Map;
  Station(){ Sigma = 20.E-4f; Sigma2 = Sigma*Sigma; }
} __attribute__ ((aligned(sizeof(Fvec_t))));
