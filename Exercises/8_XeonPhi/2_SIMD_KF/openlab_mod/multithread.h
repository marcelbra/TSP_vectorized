...
void TMyObserver::FInit()
{
  ...
  for (int iProc=0; iProc<2; iProc++){
    for(int i=0; i<6; ++i)
    {
      threadNumberToCpuMap[2*i+0 + iProc*12] = 2*i + iProc;
      threadNumberToCpuMap[2*i+1 + iProc*12] = 2*i + 12 + iProc;
    }
  }
#endif

#ifdef KNF //mic
  for(int iThread=0; iThread<119; iThread++)
      threadNumberToCpuMap[iThread] = iThread+1;
  threadNumberToCpuMap[119] = 0;
#endif
...
void TMyObserver::on_scheduler_entry(bool Is_worker)
{
  ...
  lock.acquire(mutex);
    ++threads_counter;
    int cpuId = threadNumberToCpuMap[threads_counter%120];
