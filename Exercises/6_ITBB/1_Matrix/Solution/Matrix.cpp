...
const int N = 1000; // matrix size. Has to be dividable by 4*NCores
  ...
  void operator()(const blocked_range<long unsigned int> &range, int cpuId = -1) const {
    for(int i = range.begin(); i != range.end(); ++i){
      for( int j = 0; j < N; j+=float_v::Size ) {
          ...
          float_v &cVec = (reinterpret_cast<float_v&>(c[i][j]));
          cVec = f(aVec);
      }
    }
  }
...
int main() {
  ...
  for( int ii = 0; ii < NIter; ii++ )
  {
    parallel_for(blocked_range<size_t>(0,N), ApplyTBB(a,c_tbb));
  }
  timerITBB.Stop();
